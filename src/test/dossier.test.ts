...
describe("score de salud", () => {
  ...
  it("sin casos con SLA no castiga: la ausencia de dato no es una falla", () => {
    const sinSla = healthScore({ ...perfecto, compliancePct: null });
    expect(sinSla).toBe(100);
  });

  it("sin casos abiertos no divide por cero", () => {
    expect(() => healthScore({ ...perfecto, openCases: 0, breached: 0 })).not.toThrow();
    expect(healthScore({ ...perfecto, openCases: 0, breached: 0 })).toBe(100);
  });
});
