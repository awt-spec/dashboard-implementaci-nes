...
import { buildStatementKpis } from "@/lib/exportAccountStatementPdf";
  ...
  return (
    <div className="p-6 sm:p-8 space-y-6 bg-white text-black">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 border" style={{ borderColor: RED }}>
        {buildStatementKpis(totals, analytics).map((k, i) => (
      ...
      {(analytics.byMonth.length > 0 || analytics.byTipo.length > 0) && (
            ...
            <div className="border" style={{ borderColor: RED }}>
              ...
              <div className="p-3 space-y-1.5">
                {analytics.byMonth.map((m) => (
                  <div key={m.ym || "sin-fecha"} className="flex items-center gap-2 text-xs">
                    <span className="w-16 shrink-0">{m.label}</span>
                    <div className="flex-1 h-2.5 bg-neutral-100 rounded-sm overflow-hidden">
                      <div className="h-full" style={{ width: `${m.pct}%`, background: RED }} />
                    </div>
                    <span className="w-20 text-right tabular-nums">{n2(m.hours)} h</span>
                    <span className="w-12 text-right tabular-nums text-neutral-500">{m.pct.toFixed(1)}%</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="p-3 space-y-1.5">
                {analytics.byTipo.map((t) => (
                  <div key={t.tipo} className="flex items-center gap-2 text-xs">
                    ...
                    <div className="flex-1 h-2.5 bg-neutral-100 rounded-sm overflow-hidden">
                      <div className="h-full" style={{ width: `${t.pct}%`, background: RED }} />
                    </div>
                    <span className="w-20 text-right tabular-nums">{n2(t.hours)} h</span>
                    <span className="w-12 text-right tabular-nums text-neutral-500">{t.pct.toFixed(1)}%</span>
                  </div>
                ))}
              </div>
            </div>
