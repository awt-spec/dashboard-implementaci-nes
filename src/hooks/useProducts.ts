...
export function useProducts(includeInactive = false) {
  return useQuery({
    ...
    queryFn: async () => {
      let q = (supabase.from("products" as any).select("*") as any);
...
export function useUpsertProduct() {
  ...
  return useMutation({
    mutationFn: async (input: Partial<Product> & { code: string; name: string }) => {
      ...
      if (input.id) {
        const { error } = await (supabase.from("products" as any).update(payload).eq("id", input.id) as any);
        if (error) throw error;
      } else {
        const { error } = await (supabase.from("products" as any).insert([{ ...payload, created_by: userData?.user?.id }]) as any);
        if (error) throw error;
      }
    },
...
export function useDeleteProduct() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await (supabase.from("products" as any).delete().eq("id", id) as any);
      if (error) throw error;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["products"] }),
  });
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await (
        supabase.from("product_modules" as any).select("*").eq("product_id", productId).order("sort_order", { ascending: true }) as any
      );
      if (error) throw error;
...
export function useUpsertProductModule() {
  ...
  return useMutation({
    mutationFn: async (input: Partial<ProductModule> & { product_id: string; code: string; name: string }) => {
      ...
      if (input.id) {
        const { error } = await (supabase.from("product_modules" as any).update(payload).eq("id", input.id) as any);
        if (error) throw error;
      } else {
        const { error } = await (supabase.from("product_modules" as any).insert([payload]) as any);
        if (error) throw error;
      }
    },
...
export function useDeleteProductModule() {
  ...
  return useMutation({
    mutationFn: async ({ id }: { id: string; productId: string }) => {
      const { error } = await (supabase.from("product_modules" as any).delete().eq("id", id) as any);
      if (error) throw error;
    },
...
export function useProductVersions(productId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      if (!productId) return [];
      const { data, error } = await (
        supabase.from("product_versions" as any).select("*").eq("product_id", productId).order("release_date", { ascending: false }) as any
      );
      if (error) throw error;
...
export function useUpsertProductVersion() {
  ...
  return useMutation({
    mutationFn: async (input: Partial<ProductVersion> & { product_id: string; version_label: string }) => {
      ...
      if (input.id) {
        const { error } = await (supabase.from("product_versions" as any).update(payload).eq("id", input.id) as any);
        if (error) throw error;
      } else {
        const { error } = await (supabase.from("product_versions" as any).insert([payload]) as any);
        if (error) throw error;
      }
    },
...
export function useDeleteProductVersion() {
  ...
  return useMutation({
    mutationFn: async ({ id }: { id: string; productId: string }) => {
      const { error } = await (supabase.from("product_versions" as any).delete().eq("id", id) as any);
      if (error) throw error;
    },
...
export function useVersionModules(versionId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      if (!versionId) return [];
      const { data, error } = await (
        supabase.from("version_modules" as any).select("module_id").eq("version_id", versionId) as any
      );
      if (error) throw error;
  ...
  return useMutation({
    mutationFn: async ({ versionId, moduleIds }: { versionId: string; moduleIds: string[] }) => {
      ...
      if (toRemove.length > 0) {
        const { error } = await (supabase.from("version_modules" as any).delete().eq("version_id", versionId).in("module_id", toRemove) as any);
        if (error) throw error;
      }
      if (toAdd.length > 0) {
        const { error } = await (supabase.from("version_modules" as any).insert(toAdd.map(m => ({ version_id: versionId, module_id: m }))) as any);
        if (error) throw error;
      }
    },
