-- ============================================================================
-- Fix get_client_account_statement: "operator does not exist: record ->> unknown"
-- ----------------------------------------------------------------------------
-- En dos jsonb_agg() el ORDER BY usaba `(row->>'campo')` sobre el alias de fila
-- (un record), y el operador JSON `->>` no aplica a records. Se cambia a ordenar
-- por la columna directa (`row.day` / `row.approved_at`). Sin otros cambios.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_client_account_statement(p_client_id text, p_period_start date, p_period_end date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(row ORDER BY row.approved_at DESC), '[]'::jsonb)
